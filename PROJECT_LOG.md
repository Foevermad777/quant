...
- **CEO 待办（P0-4，唯一未落地项，需 sudo）**：`sudo pmset repeat wakeorpoweron MTWRFS 05:05:00`（覆盖美股周二~六 05:10 晨跑——07-11 丢失的正是这个窗口；执行后 `pmset -g sched` 验证）。傍晚 17:58 A股窗口维持"插电+开盖"纪律即可（已证实该状态 powerd 不深睡）；如需硬保险，P1 再做晨跑 wrapper 自动登记当日 17:55 一次性 wake。另：确认新根稳定一天后删旧根。
