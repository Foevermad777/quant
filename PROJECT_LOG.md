...
- **全仓同类 bug 彻查（CEO 指令，三路对抗审计 + 定向核查，当日修复）**：按本周 bug 家族（死状态字段/时间边界/CN-US 不对称/静默吞错/误导 reason）三个并行审计 agent 扫全仓，产出 7+9+9 项发现，实锤 10 项当日全修：
  - **[高] CN reader 无股票池过滤（write+read 双侧漏）**：US reader 有 `stock_code in (pool)`，CN 完全没有——`decision_signals` 已存在池外信号 #109(588200)，一旦 `stock_daily` 出现它的 bar 且过 S1 即真实买入（当前仅被 `gate_accepted=0` 顺带拦住，第三次"侥幸兜住"）。修复：`SignalReader` 增 `stock_pool`（默认 CN 五股池），`active_signals_before` 双路径/`bars_on`/`signals_between` 全部过滤；engine/shadow/redteam/weekly_review 调用点穿线 `config.stock_pool`；写侧 `run_executor_daily.sh` 的 G5 completion 增 `--stock-code` 五股 allowlist。
  - **[高] DSA wrapper 看门狗单段 sleep 跨系统睡眠失效**（07-12 复盘遗留未修项，本次定位并修复）：`run_with_timeout` 用单个 `sleep N`，macOS 睡眠时挂起计时——07-18 那个 5244s 挂死正是这样活过 1200s 预算。两侧 wrapper 改为 `date +%s` 墙钟 deadline 轮询（5s 步进，唤醒后立即补判）。
  - **[中] `expire_stale` 本地日期 vs UTC `expires_at`**（我 07-21 引入的 bug，审计抓回）：北京 00:00-08:00 本地日期比 UTC 超前一天，US 晨跑 05:30 的 sweep 会把当天最后一个有效交易日的信号提前翻 'expired'，5 分钟后的引擎重放读不到。改用 `datetime.utcnow().date()`。
  - **[中] CN 缺 temporal 可用性判定**：US 用 `data_asof<exec + completed<开盘` 的点时判定，CN 只有日历日规则——CN 可能放进 `data_asof>=exec` 的前视行。CN 补齐镜像判定（CN 09:30 开盘），legacy 日历规则保留为无 temporal 列时的回退。生产等价探针：近 6 个执行日新旧候选集 IDENTICAL（纯防御收紧）。
  - **[中] `run_us_g5_completion.sh` 漏 `--market us`**：completion 只按股票码筛选，防线不对称。补上。
  - **[中] CN engine 开仓门 `analysis_count<=0` 一票否决**：DSA 当日失败时，前日的有效挂单计划也被跳过；US 是 `and not open_candidates`。对齐 US。一个既有测试依赖旧门的意外遮蔽（止损次日重买被挡），已按新语义修正测试。
  - **[中] dashboard `_UNEXPIRED_PREDICATE` localtime vs UTC**（我 07-21 引入，审计抓回）：北京 00:00-08:00 提前一天误报过期。改 `date('now')` UTC 口径；`analysis_24h` 窗口反向 bug（analysis_history 是本地时间戳，UTC now 使 24h 实为 ~32h）同修。
  - **[低] US reader 缺 busy_timeout**（对称补齐）；**[低] weekly_review 两侧增 `outcome_evaluator_stalled` 可见性指标**（`decision_signal_outcomes.eval_status` 外部写入、当前空表，防"评估器卡死"伪装成"无数据"——审计确认的唯一同构死状态隐患）。
  - **判定安全项**：`expire_stale` 对 redteam/weekly/测试零破坏（temporal 审计只数行不看 status）；pending_exits 生命周期正确（停牌重试是设计而非泄漏，不动）；order_attempts/signal_events 为 append-only 日志无状态语义；backfill 窗口算术无重复/跳日。**记录在案的潜在项（未改）**：reader/engine 的 `date(expires_at)`(UTC) vs 交易日比较在当前 launchd 定时下无偏差，若未来跑批时间漂移需改为市场时区换算；`_as_utc` 对 naive completed_at 按本地解释与 UTC 存储不一致（同属定时钉死下无影响）；backfill 若指向 US 池会因本地日≠交易日误报（当前仅 CN 调用）。
  - **验证**：新守卫 7 个（CN 池过滤×3、CN temporal、engine 无分析日开仓、US busy_timeout、隔离测试补池参）；全量 147 + ops 18 全绿；A/B 生产重放逐字不变（CN 4 笔 +0.95% / US 6 笔 +0.79%）；等价探针 6 日 IDENTICAL。
