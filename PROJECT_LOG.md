...
7. **执行器买卖全链路未经真实数据端到端验证**：fill/记账/止盈止损/平仓仅单测证明，live 从未跑通一笔（承接 M7/M8 R2 平仓未实现）。**待办：受控验证**——用隔离 fixture（合成一条入场区套住某历史 bar 的信号，独立 db，不重跑 DSA、不碰 live 库）跑 `run_day` 证明成交→记账→快照→平仓机制，**杜绝 look-ahead**；历史策略回测走既有 OOS 口径（`runtime_data/oos/`，point-in-time），绝不靠对旧日期重跑 DSA。
