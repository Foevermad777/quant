...
if /usr/bin/caffeinate -i "${PYTHON_BIN}" -m executor.discipline_completion --all-active --workers 4 --timeout-seconds 30 --fallback-provider deepseek --fallback-model deepseek-chat --fallback-timeout-seconds 20 --slow-threshold-ms 15000 --primary-failure-threshold 2 --retries 1 --retry-delay-seconds 1 >> "${G5_COMPLETION_LOG}" 2>&1; then
