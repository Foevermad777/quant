...
from executor.us.config_us import QUANT_DIR, UsExecutorConfig
from executor.us.ledger_us import UsPaperLedger
from executor.us.signal_reader_us import UsSignalReader, parse_date, parse_datetime
from executor.us.time_guard_us import NewsTimingAudit, bar_available_at, classify_news_for_attribution
...
def _pool_placeholders(config: UsExecutorConfig) -> str:
    return ",".join("?" for _ in config.stock_pool)


def load_signal_stats(reader: UsSignalReader, config: UsExecutorConfig, start: date, end: date) -> Dict[str, Any]:
    stats: Dict[str, Any] = {
        "signal_count": 0,
        "outcome_by_horizon": [],
    ...
    placeholders = _pool_placeholders(config)
    pool_params = (config.market, *config.stock_pool)
...
def load_paper_stats(ledger: UsPaperLedger, start: date, end: date) -> Dict[str, Any]:
    ...
    stop_sells = sum(1 for row in sell_trades if row["reason"] == "stop_loss")
...
def load_ledger_gaps(ledger: UsPaperLedger, start: date, end: date) -> List[sqlite3.Row]:
...
def load_news_timing_audit(
    reader: UsSignalReader,
    config: UsExecutorConfig,
    start: date,
    end: date,
) -> List[NewsTimingAudit]:
    placeholders = _pool_placeholders(config)
    with _connect_ro(reader.db_path) as conn:
        table = conn.execute(
        ...
        rows = conn.execute(
            ...
            """,
            (start.isoformat(), end.isoformat(), config.market, *config.stock_pool),
        ).fetchall()
...
def load_benchmarks(reader: UsSignalReader, config: UsExecutorConfig, start: date, end: date) -> Dict[str, Any]:
    equal_weight = _equal_weight_return(reader, config, start, end)
    aligned_start = parse_date(equal_weight.get("start_date")) if equal_weight.get("available") else None
    aligned_end = parse_date(equal_weight.get("end_date")) if equal_weight.get("available") else None
    return {
        "spy": _spy_return(config, aligned_start or start, aligned_end or end),
        "equal_weight": equal_weight,
    }


def _spy_return(config: UsExecutorConfig, start: date, end: date) -> Dict[str, Any]:
    errors: List[str] = []
    for code in config.benchmark_codes:
        local_result = _benchmark_from_bars(
            code=code,
            bars=[
                {"date": row["date"], "open": row["open"], "close": row["close"]}
                for row in _bars_between(config.dsa_db_path, code, start, end)
            ],
        ...
        if local_result is not None:
            return local_result
        try:
            remote_result = _benchmark_from_bars(
                code=code,
                bars=_fetch_stooq_bars(code, start, end),
                source="Stooq daily CSV, gross price return without fees/slippage",
            )
            if remote_result is not None:
                return remote_result
        except Exception as exc:
            errors.append(f"{code}/stooq: {type(exc).__name__}: {exc}")
    return {
        "available": False,
        "reason": "No SPY bars available from local DSA or Stooq."
        + (f" Errors: {'; '.join(errors[:3])}" if errors else ""),
        "source": "DSA stock_daily / Stooq daily CSV",
    }
...
def _fetch_stooq_bars(code: str, start: date, end: date) -> List[Dict[str, Any]]:
    ...
    for row in csv.DictReader(io.StringIO(text)):
        ...
        if not row.get("Date") or open_price is None or close_price is None:
            continue
        rows.append({"date": row["Date"], "open": open_price, "close": close_price})
    return rows
...
def _equal_weight_return(reader: UsSignalReader, config: UsExecutorConfig, start: date, end: date) -> Dict[str, Any]:
    ...
    if not common_dates:
        return {
            "available": False,
            "reason": "No common US stock_pool bars in review range.",
...
def closed_signal_reflections(paper_stats: Dict[str, Any], benchmarks: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    spy_return = spy.get("return") if spy.get("available") else None
    equal_weight_return = equal_weight.get("return") if equal_weight.get("available") else None
    reflections = []
        ...
        reflections.append(
            {
                ...
                "alpha_vs_spy": realized_return - spy_return
                if realized_return is not None and spy_return is not None
                else None,
                "alpha_vs_equal_weight": realized_return - equal_weight_return
                if realized_return is not None and equal_weight_return is not None
                else None,
            }
        )
    return reflections
...
def render_report(
    *,
    ...
    lines = [
        ...
        "## US Signal 面",
        "",
        f"- US signals created in range: {signal_stats['signal_count']}",
        ...
        "## 双基线对比",
        "",
        _render_benchmark("SPY", benchmarks["spy"]),
        _render_benchmark("Equal-weight US pool buy/hold", benchmarks["equal_weight"]),
        "",
        "## 统计诚实性",
        "",
        ...
        "- Baseline method: gross price return from first aligned open to last aligned close, without fees or slippage.",
        "",
        "## 延迟回填反思",
        "",
...
def _render_outcome_by_horizon(rows: Sequence[sqlite3.Row]) -> str:
    if not rows:
        return "- Outcome win rates T+1/T+3/T+5/T+10: unavailable, no completed US DSA outcomes in range."
    parts = []
    for row in rows:
        total = int(row["total"])
...
def _render_outcome_by_action_confidence(rows: Sequence[sqlite3.Row]) -> str:
    if not rows:
        return "- Outcome by action/confidence: unavailable, no completed US DSA outcomes in range."
    parts = []
    for row in rows:
        total = int(row["total"])
        wins = int(row["wins"] or 0)
...
def _render_reflections(reflections: Sequence[Dict[str, Any]]) -> str:
    if not reflections:
        return "- No closed US paper trades in range yet."
    lines = []
    for item in reflections:
        lines.append(
            ...
            f"return={_fmt_pct(item['realized_return'])} alpha_vs_spy={_fmt_pct(item['alpha_vs_spy'])} "
            f"alpha_vs_equal_weight={_fmt_pct(item['alpha_vs_equal_weight'])}"
        )
    return "\n".join(lines)
...
def _render_news_timing_audit(rows: Sequence[NewsTimingAudit]) -> str:
    if not rows:
        return "- No same-anchor-day US news timing evidence to audit in range."
    lines = [
...
def _render_data_gaps(rows: Sequence[sqlite3.Row]) -> str:
    if not rows:
        return "- No US executor data gaps recorded in range."
    return "\n".join(
...
def build_report(start: date, end: date, config: Optional[UsExecutorConfig] = None) -> str:
    config = config or UsExecutorConfig()
    reader = UsSignalReader(
        config.dsa_db_path,
        config.disciplined_db_path,
        stock_pool=config.stock_pool,
        market=config.market,
        use_disciplined_signals=config.use_disciplined_signals,
    )
    ledger = UsPaperLedger(config.ledger_db_path, config=config)
    ledger.initialize()
    signal_stats = load_signal_stats(reader, config, start, end)
    ...
    news_timing_audit = load_news_timing_audit(reader, config, start, end)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate weekly US quant review markdown.")
    ...
    output = Path(args.output) if args.output else QUANT_DIR / f"WEEKLY_REVIEW_US_{end:%Y%m%d}.md"
