...
from executor.us.config_us import PROJECT_ROOT, UsExecutorConfig
from executor.us.engine_us import UsPaperEngine
from executor.us.ledger_us import UsPaperLedger
from executor.us.oos_builder_us import DEFAULT_US_R1_CODES
...
class ValidationManifest:
    ...
    oos_stock_pool: tuple[str, ...]
    use_disciplined_signals: bool
    bar_available_timezone: str
    bar_available_time: str
...
def build_manifest(config: UsExecutorConfig, *, oos_stock_pool: Sequence[str]) -> ValidationManifest:
    return ValidationManifest(
        ...
        sec_fee_rate=config.sec_fee_rate,
        stock_pool=tuple(str(code) for code in config.stock_pool),
        oos_stock_pool=tuple(str(code) for code in oos_stock_pool),
        use_disciplined_signals=bool(config.use_disciplined_signals),
        bar_available_timezone=config.bar_available_timezone,
        bar_available_time=config.bar_available_time,
    )


def run_validation(
    config: Optional[UsExecutorConfig] = None,
    *,
    ...
    oos_stock_pool: Optional[Sequence[str]] = None,
) -> RedTeamValidationResult:
    config = config or UsExecutorConfig()
    resolved_oos_stock_pool = tuple(oos_stock_pool or DEFAULT_US_R1_CODES)
    ...
    return RedTeamValidationResult(
        manifest=build_manifest(config, oos_stock_pool=resolved_oos_stock_pool),
        oos=check_oos_gate(config, oos_start, oos_end, stock_pool=resolved_oos_stock_pool),
...
def resolve_review_window(
    ...
    if start is not None and end is not None:
        return start, end
    dates = _trading_dates(config.dsa_db_path, config.stock_pool, min_date=None, max_date=None)
    if not dates:
...
def check_oos_gate(
    ...
    *,
    ...
    codes = tuple(stock_pool or config.stock_pool)
...
def audit_temporal_gate(
    ...
    *,
    ...
    dates = _trading_dates(config.dsa_db_path, config.stock_pool, min_date=start, max_date=end)
    if not dates and start <= end:
        dates = [start]
    raw_reader = UsSignalReader(
        config.dsa_db_path,
        stock_pool=config.stock_pool,
        market=config.market,
        use_disciplined_signals=False,
    )
    disciplined_reader = UsSignalReader(
        config.dsa_db_path,
    ...
    slices = walk_forward_slices(
        _trading_dates(config.dsa_db_path, config.stock_pool, min_date=None, max_date=None),
        train_days,
        test_days,
    )
    status = "pass"
...
def run_stress_scenarios(
    ...
    *,
    ...
    scenarios = [
        (
            "base",
            config.open_slippage_multiplier,
        ...
        (
            "double_all_friction",
            config.open_slippage_multiplier * 2,
            config.commission_per_share * 2,
            config.min_commission * 2,
            config.sec_fee_rate * 2,
        ),
    ]
    results: list[StressScenarioResult] = []
    disciplined_db_path = config.disciplined_db_path or config.ledger_db_path
    with tempfile.TemporaryDirectory(prefix="quant_us_redteam_stress_") as tmpdir:
        for name, slippage_multiplier, commission_per_share, min_commission, sec_fee_rate in scenarios:
            ...
            engine = UsPaperEngine(scenario_config)
...
def render_markdown(result: RedTeamValidationResult) -> str:
    lines = [
        f"# US R1-R3 Red-Team Validation {result.manifest.generated_at}",
        "",
        ...
        f"- Execution stock pool: `{', '.join(result.manifest.stock_pool)}`",
        f"- R1 OOS stock pool: `{', '.join(result.manifest.oos_stock_pool)}`",
        f"- Market clock: `{result.manifest.bar_available_timezone}` regular close `{result.manifest.bar_available_time}`",
        "",
        "## R1 OOS Blind Gate",
        "",
    ...
    lines.extend(["", "| code | news_min_date | news_max_date | news_items |", "| --- | --- | --- | ---: |"])
    for row in result.oos.rows:
        lines.append(
    ...
    lines.extend(["", "## Verdict", "", _verdict(result)])
    return "\n".join(lines) + "\n"
...
def _stress_result_from_ledger(
    ...
    *,
    ...
    diagnostics = [
        _buy_diagnostic(scenario, UsSignalReader(config.dsa_db_path, stock_pool=config.stock_pool), row)
        for row in trades
        if row["side"] == "buy"
    ]
    return StressScenarioResult(
        ...
        total_commissions=round(sum(float(row["fees"]) for row in trades), 2),
        total_regulatory_fees=round(sum(float(row["taxes"]) for row in trades), 2),
...
def _buy_diagnostic(scenario: str, reader: UsSignalReader, trade: sqlite3.Row) -> BuyDiagnostic:
...
def _trading_dates(
    ...
    *,
    ...
    if not db_path.exists() or not stock_pool:
        return []
    predicates = [f"code in ({','.join('?' for _ in stock_pool)})"]
    params: list[str] = [str(code) for code in stock_pool]
...
def _news_coverage(conn: sqlite3.Connection, code: str, start: date, end: date) -> sqlite3.Row:
    ...
    if table is None:
        return conn.execute("select null as min_date, null as max_date, 0 as count").fetchone()
    return conn.execute(
        """
...
def _verdict(result: RedTeamValidationResult) -> str:
    ...
    if result.oos.status == "pass" and result.temporal.status == "pass" and all(item.status == "pass" for item in result.stress):
        return "US R1-R3 pass for this configured dataset and replay window."
    return "US R1-R3 are not cleared. Treat current data as engineering evidence only, not alpha or tradability evidence."
