...
class UsPaperLedger:
    def __init__(self, db_path: Path, config: Optional[UsExecutorConfig] = None) -> None:
        self.db_path = Path(db_path)
        self.config = config or UsExecutorConfig(ledger_db_path=self.db_path, disciplined_db_path=self.db_path)
    ...
    def _apply_buy(
        ...
        *,
        ...
        conn.execute(
            """
            update positions
            set quantity = ?, old_quantity = ?, avg_cost = ?, stop_loss = coalesce(?, stop_loss),
                target_price = coalesce(?, target_price), source_signal_id = coalesce(?, source_signal_id),
                updated_at = ?
            where stock_code = ?
            """,
            (new_qty, new_qty, new_avg, stop_loss, target_price, fill.signal_id, now, fill.stock_code),
        )
    ...
    def _apply_sell(self, conn: sqlite3.Connection, fill: TradeFill) -> None:
        ...
        conn.execute(
            ...
            where stock_code = ?
            """,
            (new_qty, new_qty, datetime.utcnow().isoformat(sep=" "), fill.stock_code),
        )
